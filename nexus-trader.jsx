...
// Returns { adx[], diPlus[], diMinus[] } — all length === candles.length
// ================================================================
const calcADX = (candles, p = 14) => {
  const n = candles.length;
  const adxArr = new Array(n).fill(null);
  const dpArr  = new Array(n).fill(null);
  const dmArr  = new Array(n).fill(null);
  if (n < p * 2 + 2) return { adx: adxArr, diPlus: dpArr, diMinus: dmArr };
  ...
  const diPlusRaw = [], diMinusRaw = [], dxArr = [];
  for (let i = p; i < n - 1; i++) {
    smTR  = smTR  - smTR / p  + trs[i];
    smPDM = smPDM - smPDM / p + plusDM[i];
    smMDM = smMDM - smMDM / p + minusDM[i];
    const dp = smTR ? (smPDM / smTR) * 100 : 0;
    const dm = smTR ? (smMDM / smTR) * 100 : 0;
    diPlusRaw.push(dp);
    diMinusRaw.push(dm);
    dxArr.push((dp + dm) ? Math.abs(dp - dm) / (dp + dm) * 100 : 0);
    dpArr[i + 1]  = dp;
    dmArr[i + 1]  = dm;
  }

  // ADX = smoothed DX
  if (dxArr.length < p) return { adx: adxArr, diPlus: dpArr, diMinus: dmArr };
  let adxVal = dxArr.slice(0, p).reduce((a, b) => a + b, 0) / p;
  const firstIdx = p * 2;
  adxArr[firstIdx] = adxVal;
  for (let k = p; k < dxArr.length; k++) {
    adxVal = (adxVal * (p - 1) + dxArr[k]) / p;
    adxArr[firstIdx + k - p + 1] = adxVal;
  }
  return { adx: adxArr, diPlus: dpArr, diMinus: dmArr };
};

// ================================================================
// STOCHASTIC OSCILLATOR — %K and %D
// ================================================================
const calcStoch = (candles, k = 14, dPeriod = 3) => {
  ...
  const stochD = stochK.map((_, i) => {
    const vals = stochK.slice(Math.max(0, i - dPeriod + 1), i + 1).filter(v => v !== null);
    return vals.length >= dPeriod ? vals.reduce((a, b) => a + b, 0) / vals.length : null;
  });
...
// ================================================================
const calcCCI = (candles, p = 20) => candles.map((_, i) => {
  if (i < p - 1) return null;
  const slice = candles.slice(i - p + 1, i + 1);
  const tps   = slice.map(c => (c.high + c.low + c.close) / 3);
  const mean  = tps.reduce((a, b) => a + b, 0) / p;
  const mad   = tps.reduce((a, v) => a + Math.abs(v - mean), 0) / p;
  return mad ? (tps[p - 1] - mean) / (0.015 * mad) : 0;
});
...
// ================================================================
// STRATEGY LIBRARY — Pre-built named strategies with eval functions
// ================================================================
const STRATEGY_LIBRARY = [
  {
    ...
    emoji: "📊",
    description: "95% of price action stays within 2σ Bollinger Bands. Enter when price breaks the lower band with RSI < 30 + ADX < 25 (confirming a range) + a bullish reversal candle to avoid catching falling knives.",
    indicators: ["BB(20,2)", "RSI(14)", "ADX(14)", "ATR(14)", "EMA(20)"],
    entryConditions: [
      "Price closes BELOW Lower Bollinger Band",
      "RSI < 30 (oversold)",
      "ADX < 25 (non-trending — ranging market)",
      "Bullish reversal candle (Hammer, Doji, Engulfing, etc.)",
    ],
    exitRules: { tp1: "50% at 20-EMA (mean)", tp2: "Remainder at Upper BB", sl: "1.5× ATR below entry" },
    targetRR: 2.2,
    evalFn: (ctx) => {
      if (!ctx.bb || !ctx.atr) return null;
      const longEntry  = ctx.price < ctx.bb.l && ctx.rsi != null && ctx.rsi < 30 && (ctx.adx == null || ctx.adx < 25) && ctx.bullishCandle;
      const shortEntry = ctx.price > ctx.bb.u && ctx.rsi != null && ctx.rsi > 70 && (ctx.adx == null || ctx.adx < 25) && ctx.bearishCandle;
      if (longEntry)  return { dir: "LONG",  sl: ctx.price - 1.5 * ctx.atr, tp1: ctx.ema20 || ctx.bb.m, tp2: ctx.bb.u };
      if (shortEntry) return { dir: "SHORT", sl: ctx.price + 1.5 * ctx.atr, tp1: ctx.ema20 || ctx.bb.m, tp2: ctx.bb.l };
      return null;
    },
  },
  {
    id: "golden_cross_trend",
    name: "Golden / Death Cross Trend",
    category: "Trend Following",
    emoji: "✨",
    description: "Trade the momentum shift when SMA20 crosses SMA50. ADX > 20 confirms a real trend is forming. High-probability setups with momentum behind them.",
    indicators: ["SMA(20)", "SMA(50)", "ADX(14)", "RSI(14)", "ATR(14)"],
    entryConditions: [
    ...
    exitRules: { tp1: "2× ATR from entry", tp2: "3.5× ATR from entry", sl: "Below SMA50" },
    targetRR: 2.5,
    evalFn: (ctx) => {
      if (!ctx.sma20 || !ctx.sma50 || !ctx.prevSma20 || !ctx.prevSma50 || !ctx.atr) return null;
      const longCross  = ctx.sma20 > ctx.sma50 && ctx.prevSma20 <= ctx.prevSma50;
      const shortCross = ctx.sma20 < ctx.sma50 && ctx.prevSma20 >= ctx.prevSma50;
      if (longCross  && ctx.rsi > 50  && ctx.price > ctx.sma20 && (ctx.adx == null || ctx.adx > 18))
        return { dir: "LONG",  sl: ctx.sma50, tp1: ctx.price + 2 * ctx.atr, tp2: ctx.price + 3.5 * ctx.atr };
      if (shortCross && ctx.rsi < 50  && ctx.price < ctx.sma20 && (ctx.adx == null || ctx.adx > 18))
        return { dir: "SHORT", sl: ctx.sma50, tp1: ctx.price - 2 * ctx.atr, tp2: ctx.price - 3.5 * ctx.atr };
      return null;
    },
  },
  {
    ...
    category: "Reversal",
    emoji: "🔄",
    description: "Price and RSI disagreeing = a hidden force pushing back. Regular bullish divergence (price LL but RSI HL) near S/R is one of the highest-probability reversal setups.",
    indicators: ["RSI(14)", "RSI Divergence", "S/R Levels", "ATR(14)"],
    entryConditions: [
      "Regular Bullish divergence detected (price lower low, RSI higher low)",
      "RSI between 20–45 at signal (oversold zone)",
      "Price near key support level",
      "Bullish confirmation candle after divergence",
    ],
    exitRules: { tp1: "1.5× ATR from entry", tp2: "3× ATR from entry", sl: "2× ATR below entry" },
    targetRR: 2.0,
    evalFn: (ctx) => {
      if (!ctx.atr) return null;
      if (ctx.hasBullDiv && ctx.rsi != null && ctx.rsi < 45 && ctx.rsi > 18 && ctx.atSR && ctx.bullishCandle)
        return { dir: "LONG",  sl: ctx.price - 2 * ctx.atr, tp1: ctx.price + 1.5 * ctx.atr, tp2: ctx.price + 3 * ctx.atr };
      if (ctx.hasBearDiv && ctx.rsi != null && ctx.rsi > 55 && ctx.rsi < 82 && ctx.atSR && ctx.bearishCandle)
        return { dir: "SHORT", sl: ctx.price + 2 * ctx.atr, tp1: ctx.price - 1.5 * ctx.atr, tp2: ctx.price - 3 * ctx.atr };
      return null;
    },
  },
  {
    ...
    category: "Breakout",
    emoji: "💥",
    description: "Low volatility precedes high volatility. When BBands narrow below their 20-bar average (squeeze), the ensuing breakout is explosive. Enter the direction of the MACD-confirmed break.",
    indicators: ["BB(20,2)", "MACD(12,26,9)", "ATR(14)", "Volume"],
    entryConditions: [
      "Bollinger Band width < 70% of 20-bar average (squeeze)",
      "Price breaks above Upper BB (long) or below Lower BB (short)",
      "MACD histogram aligns with breakout direction",
      "Volume spike on breakout candle (> 1.5× average)",
    ],
    exitRules: { tp1: "1× BB width above entry", tp2: "1.8× BB width above entry", sl: "Middle BB band" },
    targetRR: 2.8,
    evalFn: (ctx) => {
      if (!ctx.bb || !ctx.squeeze) return null;
      const bbW = ctx.bb.u - ctx.bb.l;
      if (ctx.price > ctx.bb.u && ctx.macdHist > 0 && ctx.volumeSpike)
        return { dir: "LONG",  sl: ctx.bb.m, tp1: ctx.price + bbW * 0.8, tp2: ctx.price + bbW * 1.5 };
      if (ctx.price < ctx.bb.l && ctx.macdHist < 0 && ctx.volumeSpike)
        return { dir: "SHORT", sl: ctx.bb.m, tp1: ctx.price - bbW * 0.8, tp2: ctx.price - bbW * 1.5 };
      return null;
    },
  },
  {
    ...
    category: "SMC",
    emoji: "🏦",
    description: "Institutions leave footprints — Order Blocks are where they placed large orders. After a Break of Structure (BOS) confirms a new trend direction, price often returns to the OB to fill remaining orders.",
    indicators: ["Order Blocks (OB)", "Fair Value Gaps (FVG)", "Break of Structure (BOS)", "ATR(14)"],
    entryConditions: [
      "Recent Break of Structure (BOS) in signal direction",
      "Price retesting a Bullish / Bearish Order Block",
      "Fair Value Gap present near entry zone",
      "RSI not in extreme opposite territory",
    ],
    exitRules: { tp1: "2.5× ATR from entry", tp2: "4× ATR from entry", sl: "1.5× ATR beyond entry" },
    targetRR: 3.0,
    evalFn: (ctx) => {
      if (!ctx.atr) return null;
      const bullBOS = ctx.smcTypes.includes("BOS_BULL");
      const bearBOS = ctx.smcTypes.includes("BOS_BEAR");
      const bullConf = ctx.smcTypes.includes("OB_BULL") || ctx.smcTypes.includes("FVG_BULL");
      const bearConf = ctx.smcTypes.includes("OB_BEAR") || ctx.smcTypes.includes("FVG_BEAR");
      if (bullBOS && bullConf && ctx.rsi != null && ctx.rsi < 65)
        return { dir: "LONG",  sl: ctx.price - 1.5 * ctx.atr, tp1: ctx.price + 2.5 * ctx.atr, tp2: ctx.price + 4 * ctx.atr };
      if (bearBOS && bearConf && ctx.rsi != null && ctx.rsi > 35)
        return { dir: "SHORT", sl: ctx.price + 1.5 * ctx.atr, tp1: ctx.price - 2.5 * ctx.atr, tp2: ctx.price - 4 * ctx.atr };
      return null;
    },
  },
  {
    ...
    category: "Mean Reversion",
    emoji: "⚖️",
    description: "Institutions benchmark against VWAP. Extreme deviations (>1.5%) in a ranging market (ADX < 25) + Stochastic extreme = high-probability snap-back trade.",
    indicators: ["VWAP", "Stochastic(14,3)", "ADX(14)", "ATR(14)"],
    entryConditions: [
      "Price > 1.5% below VWAP (oversold extension)",
      "Stochastic %K < 25 (long) or > 75 (short)",
      "ADX < 25 (ranging, not trending)",
      "Volume declining (exhaustion of move)",
    ],
    exitRules: { tp1: "VWAP level", tp2: "VWAP + 0.5× ATR", sl: "1× ATR beyond entry" },
    targetRR: 2.0,
    evalFn: (ctx) => {
      if (!ctx.vwap || !ctx.atr) return null;
      const dev = (ctx.price - ctx.vwap) / ctx.vwap * 100;
      if (dev < -1.5 && ctx.stochK != null && ctx.stochK < 25 && (ctx.adx == null || ctx.adx < 25))
        return { dir: "LONG",  sl: ctx.price - ctx.atr, tp1: ctx.vwap, tp2: ctx.vwap + ctx.atr * 0.5 };
      if (dev > 1.5  && ctx.stochK != null && ctx.stochK > 75 && (ctx.adx == null || ctx.adx < 25))
        return { dir: "SHORT", sl: ctx.price + ctx.atr, tp1: ctx.vwap, tp2: ctx.vwap - ctx.atr * 0.5 };
      return null;
    },
  },
];

// ── Run a strategy definition against candle array — returns trade array ──
const runStrategyBacktest = (candles, strat) => {
  const n = candles.length;
  if (n < 60) return [];
  const cls     = candles.map(c => c.close);
  const rsiArr  = calcRSI(cls);
  const macdObj = calcMACD(cls);
  const bbArr   = calcBB(cls);
  const atrArr  = calcATR(candles);
  const s20arr  = sma(cls, 20);
  const s50arr  = sma(cls, 50);
  const e20arr  = ema(cls, 20);
  const vwapArr = calcVWAP(candles);
  const stochObj= calcStoch(candles);
  const adxObj  = calcADX(candles);
  const smcSigs = detectSMC(candles);
  const srLvls  = detectSR(candles);
  const divs    = detectDivergence(candles, rsiArr, macdObj.hist);
  const pats    = detectCandlePatterns(candles);
  const bbWArr  = bbArr.map(b => b.u ? (b.u - b.l) / (b.m || 1) * 100 : 0);

  const trades = [];
  let skipUntil = 0;
  for (let i = 50; i < n - 3; i++) {
    if (i < skipUntil) continue;
    const c     = candles[i];
    const price = c.close;
    const rsi   = rsiArr[i];
    const bb    = bbArr[i];
    const atr   = atrArr[i] || price * 0.012;
    const adx   = adxObj.adx[i];
    const stochK= stochObj.stochK[i];
    const mHist = macdObj.hist[i];
    const sma20v= s20arr[i];
    const sma50v= s50arr[i];
    const ema20v= e20arr[i];
    const vwap  = vwapArr[i];
    if (!rsi || !bb || !atr) continue;

    const prevSma20 = s20arr[i - 1];
    const prevSma50 = s50arr[i - 1];

    const recentVols = candles.slice(Math.max(0, i - 20), i).map(x => x.volume || 0);
    const avgVol     = recentVols.reduce((a, b) => a + b, 0) / (recentVols.length || 1);
    const volumeSpike= (c.volume || 0) > avgVol * 1.5;

    const recentBBW  = bbWArr.slice(Math.max(0, i - 20), i);
    const avgBBW     = recentBBW.length ? recentBBW.reduce((a, b) => a + b, 0) / recentBBW.length : 3;
    const squeeze    = bbWArr[i] < avgBBW * 0.72;

    const barPats    = pats.filter(p => p.i === i);
    const bullishCandle = barPats.some(p => p.sig === "bullish") || c.close > c.open;
    const bearishCandle = barPats.some(p => p.sig === "bearish") || c.close < c.open;

    const recentSMC  = smcSigs.filter(s => s.i >= i - 5 && s.i <= i);
    const smcTypes   = recentSMC.map(s => s.type);
    const atSR       = srLvls.some(l => Math.abs(price - l.p) / price < 0.013);
    const hasBullDiv = divs.some(d => d.type.startsWith("bull") && d.idx2 >= i - 5 && d.idx2 <= i);
    const hasBearDiv = divs.some(d => d.type.startsWith("bear") && d.idx2 >= i - 5 && d.idx2 <= i);

    const ctx = { price, rsi, bb, atr, adx, stochK, macdHist: mHist, sma20: sma20v, sma50: sma50v, ema20: ema20v, prevSma20, prevSma50, vwap, bullishCandle, bearishCandle, volumeSpike, squeeze, smcTypes, atSR, hasBullDiv, hasBearDiv };
    const signal = strat.evalFn(ctx);
    if (!signal) continue;

    const entry = price;
    let exitPnl = 0, exitReason = "TIME", barsHeld = 0;
    for (let j = i + 1; j < Math.min(i + 25, n); j++) {
      const fc = candles[j]; barsHeld = j - i;
      if (signal.dir === "LONG") {
        if (fc.low  <= signal.sl)  { exitPnl = (signal.sl  - entry) / entry * 100; exitReason = "SL";  break; }
        if (fc.high >= signal.tp1) { exitPnl = (signal.tp1 - entry) / entry * 100; exitReason = "TP1"; break; }
      } else {
        if (fc.high >= signal.sl)  { exitPnl = (entry - signal.sl)  / entry * 100; exitReason = "SL";  break; }
        if (fc.low  <= signal.tp1) { exitPnl = (entry - signal.tp1) / entry * 100; exitReason = "TP1"; break; }
      }
    }
    if (exitReason === "TIME") {
      const exitPrice = candles[Math.min(i + 10, n - 1)].close;
      exitPnl = signal.dir === "LONG" ? (exitPrice - entry) / entry * 100 : (entry - exitPrice) / entry * 100;
    }
    trades.push({ date: c.date, dir: signal.dir, entry, sl: signal.sl, tp: signal.tp1, pnl: exitPnl, exitReason, barsHeld, hit: exitPnl > 0 });
    skipUntil = i + Math.max(barsHeld, 3); // no overlapping trades
  }
  return trades;
};
...
// STRATEGY LAB PANEL
// ================================================================
function StrategyLabPanel({ candles, ticker, tf, groqKey, groqModel }) {
  const [selected, setSelected]   = useState(null);
  const [running, setRunning]     = useState(false);
  const [results, setResults]     = useState({});   // { [stratId]: {trades,summary} }
  const [aiInsight, setAiInsight] = useState({});   // { [stratId]: string }
  const [loadingAI, setLoadingAI] = useState(null); // stratId currently loading
  const [detailTab, setDetailTab] = useState("trades"); // trades | info

  const run = async (strat) => {
    if (!candles.length || running) return;
    setSelected(strat.id);
    setRunning(true);
    await new Promise(r => setTimeout(r, 20)); // yield render
    try {
      const trades = runStrategyBacktest(candles, strat);
      const wins   = trades.filter(t => t.hit).length;
      const losses = trades.filter(t => !t.hit).length;
      const tp1s   = trades.filter(t => t.exitReason === "TP1").length;
      const sls    = trades.filter(t => t.exitReason === "SL").length;
      const totalPnl   = trades.reduce((a, t) => a + t.pnl, 0);
      const avgWin     = wins   ? trades.filter(t => t.hit).reduce((a, t) => a + t.pnl, 0) / wins   : 0;
      const avgLoss    = losses ? Math.abs(trades.filter(t => !t.hit).reduce((a, t) => a + t.pnl, 0) / losses) : 0;
      const profitFactor = avgLoss > 0 ? (avgWin * wins) / (avgLoss * losses) : wins > 0 ? 99 : 0;
      const expectancy = trades.length ? totalPnl / trades.length : 0;
      // Simple equity curve
      let eq = 100, maxEq = 100, maxDD = 0;
      const equityCurve = [100];
      trades.forEach(t => { eq *= (1 + t.pnl / 100); equityCurve.push(+eq.toFixed(2)); maxEq = Math.max(maxEq, eq); maxDD = Math.max(maxDD, (maxEq - eq) / maxEq * 100); });
      const summary = {
        total: trades.length, wins, losses, tp1s, sls,
        ...
        finalEq: eq.toFixed(2),
        equityCurve,
      };
      setResults(prev => ({ ...prev, [strat.id]: { trades, summary } }));
    } finally { setRunning(false); }
  };

  const askAI = async (strat) => {
    if (!groqKey) return;
    const res = results[strat.id];
    if (!res) return;
    setLoadingAI(strat.id);
    try {
      const s = res.summary;
      const last5 = res.trades.slice(-5).map(t => `${t.date} ${t.dir} ${t.pnl >= 0 ? "+" : ""}${t.pnl.toFixed(2)}%(${t.exitReason})`).join(", ");
      const prompt = `You are a professional quant trading strategist. Analyze this backtest of "${strat.name}" strategy.

Asset: ${ticker} | Timeframe: ${tf.split("|")[0]} | Candles tested: ${candles.length}
Strategy: ${strat.description}
Entry conditions: ${strat.entryConditions.join("; ")}
Stop-loss rule: ${strat.exitRules.sl}
Take-profit rules: TP1=${strat.exitRules.tp1} | TP2=${strat.exitRules.tp2}
Target R:R ratio: ${strat.targetRR}

Backtest results:
• Signals: ${s.total} | Win rate: ${s.winRate}% | Total P&L: ${s.totalPnl}%
• Avg Win: +${s.avgWin}% | Avg Loss: -${s.avgLoss}% | Expectancy: ${s.expectancy}%
• Profit Factor: ${s.profitFactor} | Max Drawdown: ${s.maxDD}%
• TP1 hits: ${s.tp1s} | SL hits: ${s.sls}
• Last 5 trades: ${last5}

Give a concise expert verdict (≤220 words):
1. Is this strategy profitable/suitable for this asset?
2. Key strengths vs weaknesses revealed by the data
3. Three specific improvements to filter losers better
4. Ideal market conditions / regimes for this setup`;

      const resp = await fetch("https://api.groq.com/openai/v1/chat/completions", {
        method: "POST",
        headers: { "Content-Type": "application/json", "Authorization": `Bearer ${groqKey}` },
        body: JSON.stringify({ model: groqModel || "llama-3.3-70b-versatile", messages: [{ role: "user", content: prompt }], temperature: 0.3, max_tokens: 450 }),
      });
      const data = await resp.json();
      if (data.error) throw new Error(data.error.message);
      setAiInsight(prev => ({ ...prev, [strat.id]: data.choices?.[0]?.message?.content || "No response." }));
    } catch (e) { setAiInsight(prev => ({ ...prev, [strat.id]: "AI Error: " + e.message })); }
    finally { setLoadingAI(null); }
  };

  const selStrat = STRATEGY_LIBRARY.find(s => s.id === selected);
  const selResult = selected ? results[selected] : null;

  const catColors = { "Mean Reversion": "#00d4ff", "Trend Following": "#00ff9d", "Reversal": "#a855f7", "Breakout": "#ffcc00", "SMC": "#ff8c00" };

  return (
    <div style={{ padding: 12, display: "grid", gridTemplateColumns: "220px 1fr", gap: 12, minHeight: 380 }}>
      {/* ── LEFT: Strategy List ── */}
      <div>
        <div style={{ fontSize: 8, color: "#e8f0f8", letterSpacing: 2, marginBottom: 8 }}>STRATEGY LIBRARY ({STRATEGY_LIBRARY.length})</div>
        {STRATEGY_LIBRARY.map(strat => {
          const res = results[strat.id];
          const isSelected = selected === strat.id;
          const wr = res?.summary?.winRate;
          const wrColor = wr >= 55 ? "#00ff9d" : wr >= 45 ? "#ffcc00" : wr ? "#ff3355" : "#6a7a9a";
          return (
            <div key={strat.id}
              onClick={() => { setSelected(strat.id); if (!res) run(strat); }}
              style={{ background: isSelected ? "#0a1a2a" : "#06060f", border: `1px solid ${isSelected ? "#00d4ff50" : "#1f2535"}`, borderRadius: 5, padding: "8px 10px", marginBottom: 5, cursor: "pointer", transition: "all .15s" }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                <div>
                  <span style={{ fontSize: 12 }}>{strat.emoji}</span>
                  <span style={{ fontSize: 9, fontWeight: "bold", color: isSelected ? "#00d4ff" : "#e8f0f8", marginLeft: 5 }}>{strat.name}</span>
                </div>
                {res && <span style={{ fontSize: 10, fontWeight: "bold", color: wrColor }}>{wr}%</span>}
              </div>
              <div style={{ fontSize: 8, color: catColors[strat.category] || "#888", marginTop: 2 }}>{strat.category}</div>
              {!res && !running && isSelected && (
                <div style={{ fontSize: 8, color: "#00d4ff", marginTop: 3 }}>↺ Running backtest...</div>
              )}
              {res && (
                <div style={{ display: "flex", gap: 8, marginTop: 3, fontSize: 8 }}>
                  <span style={{ color: "#00ff9d" }}>+{res.summary.avgWin}%W</span>
                  <span style={{ color: "#ff3355" }}>-{res.summary.avgLoss}%L</span>
                  <span style={{ color: "#e8f0f8" }}>{res.summary.total}t</span>
                </div>
              )}
            </div>
          );
        ...
        {candles.length < 60 && (
          <div style={{ fontSize: 8, color: "#ffcc0080", marginTop: 6, lineHeight: 1.6 }}>⚠ Fetch data first (need ≥60 candles)</div>
        )}
      </div>
      ...
      <div>
        {!selStrat ? (
          <div style={{ color: "#6a7a9a", fontSize: 10, lineHeight: 1.9, padding: 10 }}>
            {["← Select a strategy to run its backtest on the current chart.","","Each strategy is defined with precise entry conditions,","stop-loss, and take-profit rules. The engine:","","→ Simulates every valid signal in your data","→ Measures win rate, expectancy, profit factor","→ Shows equity curve and trade-by-trade results","→ AI analyses the results and suggests improvements","","Indicators now available:","RSI • MACD • Bollinger Bands • ATR • ADX (NEW)","Stochastic (NEW) • CCI (NEW) • VWAP • SMC • Divergences"].map((l, i) => <div key={i} style={{ color: l.startsWith("→") ? "#00d4ff80" : l.startsWith("RSI") ? "#a855f780" : "#6a7a9a" }}>{l}</div>)}
          </div>
        ) : (
          <div>
            {/* Strategy header */}
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 10 }}>
              <div>
                <div style={{ fontSize: 13, fontWeight: "bold", color: "#e8f0f8" }}>{selStrat.emoji} {selStrat.name}</div>
                <div style={{ fontSize: 9, color: catColors[selStrat.category] || "#888", marginTop: 2 }}>{selStrat.category} • R:R target {selStrat.targetRR}:1</div>
              </div>
              <div style={{ display: "flex", gap: 6 }}>
                <button onClick={() => run(selStrat)} disabled={running || !candles.length}
                  style={{ background: "#00ff9d18", border: "1px solid #00ff9d40", color: "#00ff9d", padding: "4px 12px", borderRadius: 4, cursor: "pointer", fontFamily: "monospace", fontSize: 9, fontWeight: "bold" }}>
                  {running ? "⟳ RUNNING..." : "▶ RUN"}
                </button>
                {selResult && (
                  <button onClick={() => askAI(selStrat)} disabled={loadingAI === selStrat.id || !groqKey}
                    style={{ background: "#a855f718", border: "1px solid #a855f740", color: "#a855f7", padding: "4px 12px", borderRadius: 4, cursor: "pointer", fontFamily: "monospace", fontSize: 9, fontWeight: "bold" }}>
                    {loadingAI === selStrat.id ? "⟳ AI..." : "🧠 ASK AI"}
                  </button>
                )}
              </div>
            </div>
            ...
            {selResult && (
              <div style={{ display: "flex", gap: 5, flexWrap: "wrap", marginBottom: 8 }}>
                {[
                  ["WIN RATE", `${selResult.summary.winRate}%`, parseFloat(selResult.summary.winRate) >= 50 ? "#00ff9d" : "#ff3355"],
                  ["TOTAL P&L", `${parseFloat(selResult.summary.totalPnl) >= 0 ? "+" : ""}${selResult.summary.totalPnl}%`, parseFloat(selResult.summary.totalPnl) >= 0 ? "#00ff9d" : "#ff3355"],
                  ["SIGNALS", selResult.summary.total, "#00d4ff"],
                  ["EXPECTANCY", `${parseFloat(selResult.summary.expectancy) >= 0 ? "+" : ""}${selResult.summary.expectancy}%`, parseFloat(selResult.summary.expectancy) >= 0 ? "#00ff9d" : "#ff3355"],
                  ["PROFIT F.", selResult.summary.profitFactor, parseFloat(selResult.summary.profitFactor) >= 1.3 ? "#00ff9d" : parseFloat(selResult.summary.profitFactor) >= 0.8 ? "#ffcc00" : "#ff3355"],
                  ["MAX DD", `-${selResult.summary.maxDD}%`, "#ff8c00"],
                  ["AVG WIN", `+${selResult.summary.avgWin}%`, "#00ff9d"],
                  ["AVG LOSS", `-${selResult.summary.avgLoss}%`, "#ff3355"],
                ].map(([l, v, c]) => (
                  <div key={l} style={{ background: "#04040c", border: "1px solid #1f2535", borderRadius: 4, padding: "3px 8px", minWidth: 60 }}>
                    <div style={{ fontSize: 7, color: "#6a7a8a" }}>{l}</div>
                    <div style={{ fontSize: 11, fontWeight: "bold", color: c }}>{v}</div>
                  </div>
                ))}
              </div>
            )}
            ...
            {/* Sub tabs */}
            <div style={{ display: "flex", gap: 0, borderBottom: "1px solid #1f2535", marginBottom: 8 }}>
              {[["info", "📋 RULES"], ["trades", "📈 TRADES"], ["equity", "💹 EQUITY"], ["ai", "🧠 AI INSIGHT"]].map(([id, label]) => (
                <button key={id} onClick={() => setDetailTab(id)}
                  style={{ background: "transparent", border: "none", borderBottom: detailTab === id ? "2px solid #00d4ff" : "2px solid transparent", color: detailTab === id ? "#00d4ff" : "#e8f0f8", padding: "4px 10px", cursor: "pointer", fontFamily: "monospace", fontSize: 8 }}>
                  {label}
                </button>
              ))}
            </div>
            ...
            {/* INFO tab */}
            {detailTab === "info" && (
              <div style={{ fontSize: 9, color: "#e8f0f8", lineHeight: 1.8 }}>
                <div style={{ color: "#7a8aa8", marginBottom: 6 }}>{selStrat.description}</div>
                <div style={{ fontSize: 8, color: "#e8f0f8", letterSpacing: 1, marginBottom: 4 }}>INDICATORS USED</div>
                <div style={{ display: "flex", flexWrap: "wrap", gap: 4, marginBottom: 8 }}>
                  {selStrat.indicators.map(ind => (
                    <span key={ind} style={{ background: "#00d4ff12", border: "1px solid #00d4ff30", color: "#00d4ff", padding: "2px 7px", borderRadius: 3, fontSize: 8 }}>{ind}</span>
                  ))}
                </div>
                <div style={{ fontSize: 8, color: "#e8f0f8", letterSpacing: 1, marginBottom: 4 }}>ENTRY CONDITIONS</div>
                {selStrat.entryConditions.map((c, i) => (
                  <div key={i} style={{ padding: "3px 0", borderBottom: "1px solid #0a0a1a", fontSize: 9 }}>
                    <span style={{ color: "#00ff9d" }}>✓</span> {c}
                  </div>
                ))}
                <div style={{ marginTop: 8, display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 6 }}>
                  {[["TAKE PROFIT 1", selStrat.exitRules.tp1, "#00ff9d"], ["TAKE PROFIT 2", selStrat.exitRules.tp2, "#00ffa0"], ["STOP LOSS", selStrat.exitRules.sl, "#ff3355"]].map(([k, v, c]) => (
                    <div key={k} style={{ background: "#04040c", border: `1px solid ${c}20`, borderRadius: 4, padding: "6px 8px" }}>
                      <div style={{ fontSize: 7, color: c, letterSpacing: 1 }}>{k}</div>
                      <div style={{ fontSize: 8, color: "#e8f0f8", marginTop: 2, lineHeight: 1.5 }}>{v}</div>
                    </div>
                  ))}
                </div>
              </div>
            ...
            {/* TRADES tab */}
            {detailTab === "trades" && selResult && (
              <div>
                {selResult.trades.length === 0 ? (
                  <div style={{ color: "#6a7a9a", fontSize: 9, padding: 10 }}>No signals found in this dataset. Try a different timeframe or asset.</div>
                ) : (
                  <div style={{ background: "#04040c", borderRadius: 4, overflow: "hidden" }}>
                    <div style={{ display: "grid", gridTemplateColumns: "54px 50px 64px 64px 60px 50px 1fr", padding: "4px 8px", borderBottom: "1px solid #1f2535" }}>
                      {["DATE", "DIR", "ENTRY", "TP/SL", "P&L", "EXIT", "BARS"].map((h, i) => (
                        <div key={i} style={{ fontSize: 7, color: "#e8f0f8" }}>{h}</div>
                      ))}
                    </div>
                    <div style={{ maxHeight: 200, overflowY: "auto" }}>
                      {selResult.trades.map((t, i) => (
                        <div key={i} style={{ display: "grid", gridTemplateColumns: "54px 50px 64px 64px 60px 50px 1fr", padding: "4px 8px", borderBottom: "1px solid #08081a", background: t.hit ? "#001508" : "#06060f" }}>
                          <span style={{ fontSize: 7, color: "#5a6a80" }}>{t.date}</span>
                          <span style={{ fontSize: 8, color: t.dir === "LONG" ? "#00ff9d" : "#ff3355" }}>{t.dir}</span>
                          <span style={{ fontSize: 8, color: "#e8f0f8" }}>{t.entry < 10 ? t.entry.toFixed(4) : t.entry.toFixed(2)}</span>
                          <span style={{ fontSize: 8, color: t.exitReason === "TP1" ? "#00ff9d" : "#ff3355" }}>{t.tp < 10 ? t.tp?.toFixed(4) : t.tp?.toFixed(2)}</span>
                          <span style={{ fontSize: 9, fontWeight: "bold", color: t.pnl >= 0 ? "#00ff9d" : "#ff3355" }}>{t.pnl >= 0 ? "+" : ""}{t.pnl.toFixed(2)}%</span>
                          <span style={{ fontSize: 8, color: t.exitReason === "SL" ? "#ff335580" : t.exitReason === "TP1" ? "#00ff9d80" : "#ffcc0080" }}>{t.exitReason}</span>
                          <span style={{ fontSize: 8, color: "#5a6a8a" }}>{t.barsHeld}</span>
                        </div>
                      ))}
                    </div>
                  </div>
            ...
            {/* EQUITY tab */}
            {detailTab === "equity" && selResult && selResult.summary.equityCurve.length > 1 && (
              <div style={{ background: "#04040c", borderRadius: 5, padding: 10 }}>
                <div style={{ fontSize: 8, color: "#e8f0f8", marginBottom: 6 }}>Equity curve — $100 start, compounded · Final: ${selResult.summary.finalEq}</div>
                <svg width="100%" height={120} viewBox="0 0 600 120" preserveAspectRatio="none">
                  {(() => {
                    const vals = selResult.summary.equityCurve;
                    const minV = Math.min(...vals), maxV = Math.max(...vals);
                    const range = maxV - minV || 1;
                    const pts = vals.map((v, i) => `${(i / (vals.length - 1)) * 600},${120 - ((v - minV) / range) * 110}`).join(" ");
                    const area = `M0,120 L${pts.split(" ").map((p, i) => i === 0 ? `0,${p.split(",")[1]}` : p).join(" L")} L600,120 Z`;
                    return (
                      <>
                        <defs>
                          <linearGradient id="eqGrad2" x1="0" y1="0" x2="0" y2="1">
                            <stop offset="0%" stopColor={parseFloat(selResult.summary.finalEq) >= 100 ? "#00ff9d" : "#ff3355"} stopOpacity={0.3} />
                            <stop offset="100%" stopColor={parseFloat(selResult.summary.finalEq) >= 100 ? "#00ff9d" : "#ff3355"} stopOpacity={0.02} />
                          </linearGradient>
                        </defs>
                        <path d={area} fill="url(#eqGrad2)" />
                        <polyline points={pts} fill="none" stroke={parseFloat(selResult.summary.finalEq) >= 100 ? "#00ff9d" : "#ff3355"} strokeWidth={1.5} />
                        <line x1={0} y1={120 - ((100 - minV) / range) * 110} x2={600} y2={120 - ((100 - minV) / range) * 110} stroke="#6a7a9a" strokeWidth={0.5} strokeDasharray="4,2" />
                      </>
                    );
                  })()}
                </svg>
            ...
            {detailTab === "ai" && (
              <div>
                {!groqKey && <div style={{ color: "#ffcc0080", fontSize: 9, padding: 10 }}>⚠ Add Groq API key in ⚙ Settings to get AI strategy analysis.</div>}
                {groqKey && !selResult && <div style={{ color: "#6a7a9a", fontSize: 9, padding: 10 }}>Run the backtest first, then click 🧠 Ask AI for analysis.</div>}
                {groqKey && selResult && !aiInsight[selStrat.id] && !loadingAI && (
                  <div style={{ padding: 10 }}>
                    <div style={{ color: "#7a8aa8", fontSize: 9, marginBottom: 8 }}>AI will analyse the backtest results and give you:</div>
                    {["A verdict on whether this strategy suits this asset", "Specific weaknesses revealed by the P&L data", "3 filter improvements to reduce losing trades", "Optimal market regime conditions"].map((l, i) => (
                      <div key={i} style={{ fontSize: 9, color: "#6a7a9a", padding: "2px 0" }}>→ {l}</div>
                    ))}
                    <button onClick={() => askAI(selStrat)} style={{ marginTop: 10, background: "#a855f718", border: "1px solid #a855f740", color: "#a855f7", padding: "6px 16px", borderRadius: 4, cursor: "pointer", fontFamily: "monospace", fontSize: 10, fontWeight: "bold" }}>
                      🧠 ANALYSE WITH AI
                    </button>
                  </div>
                )}
                {loadingAI === selStrat.id && (
                  <div style={{ color: "#a855f7", fontSize: 9, padding: 10, animation: "pulse 1.5s infinite" }}>⟳ AI is analysing the backtest data...</div>
                )}
                {aiInsight[selStrat.id] && (
                  <div style={{ background: "#08081a", border: "1px solid #a855f730", borderRadius: 5, padding: 12, fontSize: 9, color: "#c8a8e8", lineHeight: 1.85, whiteSpace: "pre-wrap" }}>
                    {aiInsight[selStrat.id]}
                  </div>
                )}
              </div>
...
export default function NexusTrader() {
  ...
  const analysis = useMemo(() => {
    ...
    return { s20, s50, s200, rsiarr, macdarr, bbarr, atrarr, patterns, smcSigs, srLvls, chartPatterns, chartData, fib, divergences, vwap, pivots, volProfile, regime, adxObj, stochObj, cciarr };
  }, [candles]);
...
${memoryCtx ? memoryCtx + "\n" : ""}Reply ONLY raw JSON: {"bias":"BULLISH"|"BEARISH"|"NEUTRAL","confidence":0-100,"summary":"...","trend":{"short":"...","medium":"...","long":"..."},"setup":{"direction":"LONG"|"SHORT"|"WAIT","entry":"...","stopLoss":"...","tp1":"...","tp2":"...","rr":"1:X","rationale":"..."},"keyLevels":["...","...","..."],"smartMoney":"...","indicators":{"rsi":"...","macd":"...","bollinger":"...","volume":"..."},"risks":"...","watch":"..."}`;
  ...
  const S = {
    wrap: { background: "linear-gradient(135deg,#04040c 0%,#060611 100%)", minHeight: "100vh", width: "100%", overflow: "hidden", color: "#e8f0f8", fontFamily: "'JetBrains Mono',monospace", fontSize: 12, boxSizing: "border-box" },
  ...
  return (
    <div style={S.wrap}>
      ...
      {/* MAIN 3-COL GRID */}
      <div style={{ display: "grid", gridTemplateColumns: "150px 1fr 230px", gap: 6, padding: 4, width: "100%", boxSizing: "border-box" }}>
        {/* LEFT */}
        <div style={{ display: "flex", flexDirection: "column", gap: 8, minWidth: 0, overflow: "hidden" }}>
          {last && analysis && (
            <div style={S.panel}>
              ...
              {(() => {
                const adxVal   = analysis.adxObj?.adx.filter(v => v != null).slice(-1)[0];
                const stochKv  = analysis.stochObj?.stochK.filter(v => v != null).slice(-1)[0];
                const cciVal   = analysis.cciarr?.filter(v => v != null).slice(-1)[0];
                return (
                  <>
                    {adxVal != null && <div style={S.stat}><span style={{ color: "#e8f0f8", fontSize: 9, letterSpacing: 1 }}>ADX(14)</span><span style={{ color: adxVal > 25 ? "#ffcc00" : "#00d4ff", fontSize: 10, fontWeight: "bold" }}>{adxVal.toFixed(1)} {adxVal > 25 ? "TREND" : "RANGE"}</span></div>}
                    {stochKv != null && <div style={S.stat}><span style={{ color: "#e8f0f8", fontSize: 9, letterSpacing: 1 }}>STOCH%K</span><span style={{ color: stochKv > 80 ? "#ff3355" : stochKv < 20 ? "#00ff9d" : "#e8f0f8", fontSize: 10, fontWeight: "bold" }}>{stochKv.toFixed(1)}</span></div>}
                    {cciVal != null && <div style={S.stat}><span style={{ color: "#e8f0f8", fontSize: 9, letterSpacing: 1 }}>CCI(20)</span><span style={{ color: cciVal > 100 ? "#ff3355" : cciVal < -100 ? "#00ff9d" : "#e8f0f8", fontSize: 10, fontWeight: "bold" }}>{cciVal.toFixed(0)}</span></div>}
                  </>
                );
              })()}
            </div>
      ...
      {/* ============================================================ */}
      <div style={{ margin: "0 4px 10px", background: "#06060f", border: "1px solid #1f2535", borderRadius: 6, overflow: "hidden", width: "calc(100% - 8px)", boxSizing: "border-box", maxWidth: "100%" }}>
        {/* Tab bar */}
        ...
        {bottomOpen && (
          <div style={{ maxHeight: 430, overflowY: "auto" }}>
            ...
            {bottomTab === "strategies" && <StrategyLabPanel candles={candles} ticker={ticker} tf={tf} groqKey={groqKey} groqModel={groqModel} />}
          </div>
        )}
      </div>
