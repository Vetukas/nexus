...
function CandleChart({ candles, s20arr, s50arr, s200arr, bbarr, srLvls, showSMA20, showSMA50, showSMA200, showBB, showSR, showVol, fibData, divergences, vwapArr, pivots, drawTool, drawings, onAddDrawing, onDeleteDrawing, volProfile }) {
  ...
  if (!n) return <div ref={ref} style={{ height: 280, display: "flex", alignItems: "center", justifyContent: "center", color: "#2a3a58", fontFamily: "monospace", fontSize: 12 }}>NO DATA — FETCH A TICKER</div>;
    ...
    <div style={{ background: "#06060f", border: "1px solid #0f1525", borderRadius: 6 }}>
      ...
      <div style={{ padding: 10 }}>
        ...
        {[
          ...
          ["POSITION VALUE",posVal    ? `$${posVal.toFixed(2)}` : "—", "#c8d4e8"],
...
function AIChatPanel({ ticker, tf, analysis, candles, groqKey, groqModel }) {
  ...
  return (
    <div style={{ background: "#06060f", border: "1px solid #0f1525", borderRadius: 6, display: "flex", flexDirection: "column" }}>
        ...
        {msgs.map((m, i) => (
          <div key={i} style={{ display: "flex", flexDirection: "column", alignItems: m.role === "user" ? "flex-end" : "flex-start" }}>
            <div style={{
              ...
              border: `1px solid ${m.role === "user" ? "#00d4ff30" : "#0f1525"}`,
              color: m.role === "user" ? "#00d4ff" : "#c8d4e8",
            }}>{m.content}</div>
          </div>
        ))}
        {loading && (
      ...
      <div style={{ display: "flex", gap: 6, padding: 8, borderTop: "1px solid #0f1525" }}>
        ...
        {keys.map(k => {
          ...
          return (
            <div key={k} onClick={() => setSelected(k)}
              style={{ background: selected === k ? "#0f2040" : "#06060f", border: `1px solid ${selected === k ? "#00d4ff40" : "#0f1525"}`, borderRadius: 5, padding: "7px 10px", marginBottom: 4, cursor: "pointer" }}>
              <div style={{ display: "flex", justifyContent: "space-between" }}>
                <span style={{ fontWeight: "bold", color: "#c8d4e8", fontSize: 11 }}>{k}</span>
          ...
          {tab === "confluence" && (
            <div>
              ...
              {["elite","high","medium","low"].map(lvl => {
                ...
                return (
                  <div key={lvl} style={{ background: isCurrent ? "#0a1a28" : "#06060f", border: `1px solid ${isCurrent ? "#00d4ff30" : "#0f1525"}`, borderRadius: 5, padding: "8px 12px", marginBottom: 5 }}>
                ...
                <>
                  ...
                  {findSimilarSignals(mem, selected.split("-")[0], selected.split("-").slice(1).join("-") || "1d", currentFV, 8).map((s, i) => (
                    <div key={i} style={{ background: s.correct?"#00150808":"#15000808", border:`1px solid ${s.correct?"#00ff9d20":"#ff335520"}`, borderRadius: 5, padding: "7px 10px", marginBottom: 4 }}>
                      ...
                      <div style={{ fontSize: 8, color: "#3a4a60" }}>
...
function WatchlistPanel({ onLoadTicker, groqKey, currentTicker }) {
  ...
  const sigC = { BUY: "#00ff9d", SELL: "#ff3355", HOLD: "#ffcc00", ERR: "#3a4a60" };
  return (
        ...
        {items.map((it, i) => {
          ...
          return (
            <div key={it.sym} onClick={() => onLoadTicker(it.sym)}
              style={{ background: isActive ? "#0f2040" : "#06060f", border: `1px solid ${isActive ? "#00d4ff50" : "#0f1525"}`, borderRadius: 6, padding: "8px 10px", cursor: "pointer" }}>
              <div style={{ display: "flex", justifyContent: "space-between" }}>
                <div>
      ...
      <div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 5, maxHeight: 310, overflowY: "auto" }}>
          ...
          {news.map((n, i) => {
            ...
            return (
              <div key={i} style={{ background: "#06060f", border: `1px solid ${it ? (it.sentiment === "bullish" ? "#00ff9d20" : it.sentiment === "bearish" ? "#ff335520" : "#0f1525") : "#0f1525"}`, borderRadius: 5, padding: "8px 10px" }}>
        ...
        <>
          ...
          {rows.map((r, i) => r.err ? (
            ...
            <div key={i} style={{ display: "grid", gridTemplateColumns: "52px 60px 56px 60px 80px 80px 80px 1fr", padding: "9px 10px", borderBottom: "1px solid #08081a", background: i % 2 === 0 ? "#06060f" : "#04040e", alignItems: "center" }}>
              <span style={{ fontWeight: "bold", color: "#c8d4e8", fontSize: 12 }}>{r.tf}</span>
              ...
              <span>
                <div style={{ width: 36, height: 4, background: "#0f1525", borderRadius: 2, marginBottom: 2 }}>
              ...
              <span style={{ fontSize: 9 }}>
                ...
                <span style={{ color: "#3a4a60" }}>{r.pats}</span>
              </span>
            </div>
          ))}
          {/* Confluence bar */}
            ...
            <div key={e.id} style={{ background: "#06060f", border: `1px solid ${e.pnl >= 0 ? "#00ff9d18" : "#ff335518"}`, borderRadius: 5, padding: "8px 12px" }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 4 }}>
                <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
                  ...
                  <span style={{ fontWeight: "bold", color: "#c8d4e8", fontSize: 12 }}>{e.sym}</span>
        ...
        <div style={{ background: "#04040c", borderRadius: 4, overflow: "hidden" }}>
          ...
          {filtered.map((r, i) => (
            <div key={r.sym} onClick={() => onLoadTicker(r.sym)}
              ...
              <span style={{ fontWeight: "bold", color: "#c8d4e8", fontSize: 12 }}>{r.sym}</span>
              <span style={{ color: "#c8d4e8", fontSize: 11, fontWeight: "bold" }}>{r.price < 10 ? r.price.toFixed(4) : r.price.toFixed(2)}</span>
              ...
              <span>
                ...
                <span style={{ fontSize: 8, color: "#3a4a60" }}>{r.score}/100</span>
              </span>
              <span style={{ fontSize: 8, color: "#3a4a60" }}>{[...r.patterns, ...r.smcSigs].slice(0, 3).join(" · ")}</span>
            </div>
          ))}
        </div>
      )}
    ...
    <div style={{ padding: 12 }}>
      ...
      {running && (
        <div style={{ marginBottom: 8 }}>
          <div style={{ width: "100%", height: 5, background: "#0f1525", borderRadius: 3 }}>
          ...
          <div style={{ maxHeight: 280, overflowY: "auto" }}>
            {results.map((r, i) => (
              <div key={i} style={{ display: "grid", gridTemplateColumns: "54px 70px 55px 40px 42px 70px 70px 60px 55px 50px 1fr", padding: "5px 8px", borderBottom: "1px solid #08081a", background: r.hit ? "#001508" : r.direction==="WAIT" ? "#06060f" : "#15000808", alignItems: "center" }}>
                ...
                <span style={{ fontSize: 7, color: "#3a4a60" }}>{[r.strongBullCandle?"★Bull":r.strongBearCandle?"★Bear":(r.candleCount||0)>0?"pat":"", r.hasDivergence?"DIV":"", r.hasOB?"OB":"", r.fibState&&r.fibState!=="none"?"Fib":"", r.srState&&r.srState!=="none"?"SR":""].filter(Boolean).join(" ")}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      {tab === "info" && (
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
          {[
            ...
            <div key={title} style={{ background: "#06060f", border: "1px solid #0f1525", borderRadius: 5, padding: "10px 12px" }}>
            ...
            paper.positions.map(p => {
              ...
              return (
                <div key={p.id} style={{ background: "#06060f", border: `1px solid ${pnlPct>=0?"#00ff9d20":"#ff335520"}`, borderRadius: 5, padding: "8px 10px", marginBottom: 5 }}>
                  <div style={{ display: "flex", justifyContent: "space-between" }}>
                    <div><span style={{ fontWeight: "bold", color: "#c8d4e8" }}>{p.sym}</span> <span style={{ color: p.dir==="LONG"?"#00ff9d":"#ff3355", fontSize: 9 }}>{p.dir}</span> <span style={{ fontSize: 8, color: "#a855f7" }}>CS:{p.confluenceScore}</span></div>
                  ...
                  <div style={{ fontSize: 8, color: "#3a4a60", marginTop: 2 }}>{p.reason} · {p.openTime}</div>
                </div>
              );
            })}
        </div>
...
${memoryCtx ? memoryCtx + "\n" : ""}Reply ONLY raw JSON: {"bias":"BULLISH"|"BEARISH"|"NEUTRAL","confidence":0-100,"summary":"...","trend":{"short":"...","medium":"...","long":"..."},"setup":{"direction":"LONG"|"SHORT"|"WAIT","entry":"...","stopLoss":"...","tp1":"...","tp2":"...","rr":"1:X","rationale":"..."},"keyLevels":["...","...","..."],"smartMoney":"...","indicators":{"rsi":"...","macd":"...","bollinger":"...","volume":"..."},"risks":"...","watch":"..."}`;
  ...
  return (
    <div style={S.wrap}>
      ...
      {/* MAIN 3-COL GRID */}
      <div style={{ display: "grid", gridTemplateColumns: "200px 1fr 280px", gap: 8, padding: 10 }}>
        {/* LEFT */}
        <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
          {last && analysis && (
            <div style={S.panel}>
      ...
      {/* ============================================================ */}
      <div style={{ margin: "0 10px 14px", background: "#06060f", border: "1px solid #0f1525", borderRadius: 6, overflow: "hidden" }}>
        {/* Tab bar */}
        <div style={{ display: "flex", background: "#04040c", borderBottom: bottomOpen ? "1px solid #0f1525" : "none" }}>
          {BOTTOM_TABS.map(tab => (
            <button key={tab.id}
